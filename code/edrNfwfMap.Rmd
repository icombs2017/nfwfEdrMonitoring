...
mainKeys <- ggplot() +
  ...
  #Left Segment
  geom_segment(aes(x = -81.86, y = 24.47, xend = -81.902, yend = 24.491), size = 0.25) +
  # Right Segment
  geom_segment(aes(x = -81.825, y = 24.47, xend = -81.781, yend = 24.491), size = 0.25) +
...
edrSites <- ggplot() +
  ...
  guides(fill = guide_legend(override.aes = list(shape = 22, color = NA, size = 4), ncol = 2, order = 1), shape = guide_legend(override.aes = list(size = 3), order = 2), color = guide_legend(override.aes = list(fill = "black", alpha = 0.1), order = 3)) +
  coord_sf(xlim = c(-81.846, -81.843), ylim = c(24.4585,  24.4599)) +
  annotation_scale(location = "bl") +
  theme_bw() +
  theme(legend.title = element_text(size = 9, face = "bold"),
        axis.ticks = element_blank(),
...
#In order for this to work... you have to "unblank" the legend from EDR sites above... working to make this better
...
```{r}
ggplot(data = sites, aes(x = long_x, y = lat_y))+geom_point()
```
