...
# APAL Data
apalData <- read_excel("../data/outplantData.xlsx", sheet = "ex-situ Apal outplants", na = c("", "N/A", "NA", 'no data', "Not Monitored"))

# ACER Data
acerData <- read_excel("../data/outplantdata.xlsx", sheet = "in-situ Acer outplants", na = c("", "N/A", "NA", 'no data', "Not Monitored"))
...
outplantAPALplot1<- ggplot(outplantAPAL, aes(x = Date, y = cum_sum))+
  ...
  geom_segment(aes(x = as.Date("2023-05-09"), y = 19363,
                   xend = as.Date("2025-01-31"), yend = 19363),
...
scale_x_date(limits = c(as.Date("2021-03-01"), as.Date("2025-01-31")), 
             date_labels = "%b-%Y", date_breaks = "3 month", expand = c(0, 0))
...
outplantAPALtab1 <- apalData %>%
  select(Date, `Site ID`, `# plugs`, Lat, Long, `Genotypes represented at the Site`, Deliverable) %>%
  mutate(numberOfGenotypes = str_count(`Genotypes represented at the Site`, ",") + 1) %>%  # this counts all of the "geno types used" section in this and adds them to a column that is "number of Genotypes"
...
outplantACERplot1<- ggplot(outplantACER, aes(x = Date, y = cum_sum))+
  ...
  geom_segment(aes(x = as.Date("2023-05-30"), y = 8100,
                   xend = as.Date("2025-01-31"), yend = 8100),
               linetype = 'solid', color = 'blue', size = 3)+ # remove this with full dataset
...
outplantACERtab1 <- acerData %>%
  select(Date, `Site ID`, `# frags`, Lat, Long, `Gentoypes Represented at the Site`, Deliverable) %>%
    mutate(numberOfGenotypes = str_count(`Gentoypes Represented at the Site`, ",") + 1) %>%  # this counts all of the "geno types used" section in this and adds them to a column that is "number of Genotypes"
  # this counts all of the "geno types used" section in this and adds them to a column that is "number of Genotypes"
  mutate(Date = as.Date(Date)) %>% 
  filter(Deliverable %in% c("NFWF-EDR Yr3", "NFWF-EDR", "NFWF-EDR Yr2")) %>% 
...
apal1MoSurvivalPlot1<- ggplot(OneMoSurvivalAPAL2, aes(x = `Site ID`, y = (`1 mo percentage`*100), fill = `1 month`, color = I('black')))+
...
oneMonthAPALtab <- oneMonthAPALtab1 %>% 
  ...
  fontsize(size = 12, part = "all") %>%
  bold(part = "header") %>%
  align(align = "center", part = "all") %>%  # Align all parts to center
  set_table_properties(width = 1, align = "center", layout = "autofit") %>%  
...
apal12MoSurvivalPlot1<- ggplot(twelveMoSurvivalAPAL2, aes(x = `Site ID`, y = (`12 mo percentage`*100), fill = `12 month`, color = I('black')))+
  geom_bar(position = "stack", stat = 'identity', width = 1)+
...
# 12 month APAL plugs
twelveMonthWeightedAverageSurvival <- (sum(twelveMonthAPALtab1$twelveMonthNumberSurvived, na.rm = TRUE)) / 
                                      (sum(twelveMonthAPALtab1$`# plugs`, na.rm = TRUE)) * 100


# 12 month APAL clusters
twelveMonthWeightedAverageClusterSurvival <- (sum(twelveMonthAPALtab1$twelveMonthClusterSurvived, na.rm = TRUE)) / 
                                             (sum(twelveMonthAPALtab1$`# plugs` / 4, na.rm = TRUE)) * 100
