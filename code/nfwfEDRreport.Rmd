...
outplantAPALtab1 <- apalData %>%
  select(Date, `Site ID`, `# plugs`, Lat, Long, `Genotypes represented at the Site`:ncol(.), Deliverable) %>%
  mutate(numberOfGenotypes = rowSums(!is.na(select(., `Genotypes represented at the Site`:ncol(.))))) %>%  # this counts all of the "geno types used" section in this and adds them to a column that is "number of Genotypes"
